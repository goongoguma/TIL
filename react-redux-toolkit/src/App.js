      ...
      <AppHeader>
        ...
        <TodoContainer>
        {
          todos.map((item, index) => {
            return (
              <TodoList key={index}>
                <TodoCheckBox type='checkbox' checked={item.completed} onChange={() => handleToggle(item.id, item.completed)} />
                <p>{item.task}</p>
                <TodoDelButton onClick={() => handleRemove(item.task)}>X</TodoDelButton>
              </TodoList>
            )
          })
        }
